...
import dev.cloudy.homes.system.Home;
...
public class HomeDeleteCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        ...
        Home home = Homes.getInstance().getHomeProvider().getHome(player, name);
        if (home == null) {
            player.sendMessage(CC.translate("&cA home with the name &e" + name + " &cdoes not exist!"));
            return true;
        }

        Homes.getInstance().getHomeProvider().removeHome(player, home);
